...
# Script để khởi chạy Backend API Bất Động Sản với Poetry
# Author: Auto-generated script

set -e  # Dừng script nếu có lỗi

echo "🏠 Đang khởi chạy Backend API Bất Động Sản..."
...
echo "🚀 Đang khởi chạy server..."
echo "📍 Server sẽ chạy tại: http://localhost:8000"
echo "📖 API Documentation: http://localhost:8000/docs"
echo "🛑 Nhấn Ctrl+C để dừng server"

# Chạy server với poetry (từ thư mục backend)
# Script start trong pyproject.toml sẽ tìm file app.py trong src/
PYTHONPATH=src poetry run uvicorn src.app:app --host 0.0.0.0 --port 8000
